...
Changes vs v4.17b (this version — spatial gauge field + thermal noise):
  ...
  - Gauge field evolved via leapfrog: ∂_t²Ax = −∂_y B + Jx, ∂_t²Ay = +∂_x B + Jy
    where B = ∂_x Ay − ∂_y Ax and Jx,Jy is the Noether supercurrent
  - Vortex placement initialises Ax, Ay with the London profile f(r)/r²
    so each vortex carries quantised magnetic flux Φ = 2π/e_W through its core
  - PML absorbing boundary extended to Ax, Ay
...
_E_W2  = E_W * E_W                 # e²  — precomputed for step()
_2E_W  = 2.0 * E_W                 # 2e  — precomputed for step()
_LAM_L = 1.0 / E_W                 # London penetration depth ≈ 6.9 grid cells
_LAM_L2 = _LAM_L * _LAM_L          # λ_L² — used in London gauge profile
...
# Gauge-field scratch (v4.18) — 9 × 256² × 4 B ≈ 2.4 MB
_step_dp1x = np.empty((N, N), dtype=np.float32)   # ∂_x p1
_step_dp1y = np.empty((N, N), dtype=np.float32)   # ∂_y p1
_step_dp2x = np.empty((N, N), dtype=np.float32)   # ∂_x p2
_step_dp2y = np.empty((N, N), dtype=np.float32)   # ∂_y p2
_step_B    = np.empty((N, N), dtype=np.float32)   # B = curl A
_step_Jx   = np.empty((N, N), dtype=np.float32)   # supercurrent x
_step_Jy   = np.empty((N, N), dtype=np.float32)   # supercurrent y
_step_Anx  = np.empty((N, N), dtype=np.float32)   # new Ax
_step_Any  = np.empty((N, N), dtype=np.float32)   # new Ay
...
# Spatial gauge field (v4.18)
Ax  = np.zeros((N, N), dtype=np.float32)
Ay  = np.zeros((N, N), dtype=np.float32)
AxP = np.zeros((N, N), dtype=np.float32)
AyP = np.zeros((N, N), dtype=np.float32)
...
def step():
    ...
    global Ax, Ay, AxP, AyP
    # global needed: augmented assignments make Python treat module arrays as local
    global _step_pot, _step_v1, _step_v2, _step_n1, _step_n2, _step_tmp
    global _step_dp1x, _step_dp1y, _step_dp2x, _step_dp2y
    global _step_B, _step_Jx, _step_Jy, _step_Anx, _step_Any

    lap_into(p1, _lap1);  lap_into(p2, _lap2)

    # ── Field gradients — used for covariant Laplacian + supercurrent ──────────
    # x = column direction, y = row direction; interior only (boundary stays 0)
    _step_dp1x[:] = 0.0;  _step_dp1y[:] = 0.0
    _step_dp2x[:] = 0.0;  _step_dp2y[:] = 0.0
    np.subtract(p1[1:-1, 2:], p1[1:-1, :-2], out=_step_dp1x[1:-1, 1:-1]); _step_dp1x[1:-1, 1:-1] *= 0.5
    np.subtract(p1[2:, 1:-1], p1[:-2, 1:-1], out=_step_dp1y[1:-1, 1:-1]); _step_dp1y[1:-1, 1:-1] *= 0.5
    np.subtract(p2[1:-1, 2:], p2[1:-1, :-2], out=_step_dp2x[1:-1, 1:-1]); _step_dp2x[1:-1, 1:-1] *= 0.5
    np.subtract(p2[2:, 1:-1], p2[:-2, 1:-1], out=_step_dp2y[1:-1, 1:-1]); _step_dp2y[1:-1, 1:-1] *= 0.5

    # Potential: λ(|φ|²−1) → _step_pot
    ...
    # Damped leapfrog — φ_{n+1} = (1+d)·φ_n − d·φ_{n-1} + DT²·F
    # Vacuum (φ=v, F=0): (1+d)·v − d·v = v — no drift.
    d = S['damp']

    # e²|A|² → _step_Anx (reused for new Ax in gauge step)
    np.multiply(Ax, Ax, out=_step_Anx)
    np.multiply(Ay, Ay, out=_step_Any);  _step_Anx += _step_Any
    _step_Anx *= _E_W2   # _step_Anx = e²|A|²

    # ── New p1: covariant D²p1 = ∇²p1 + 2e(Ax·∂_xp2 + Ay·∂_yp2) − e²|A|²·p1 ──
    np.multiply(C2, _lap1, out=_step_n1)
    np.multiply(Ax, _step_dp2x, out=_step_tmp);  _step_tmp *= (_2E_W * C2);  _step_n1 += _step_tmp
    np.multiply(Ay, _step_dp2y, out=_step_tmp);  _step_tmp *= (_2E_W * C2);  _step_n1 += _step_tmp
    np.multiply(_step_Anx, p1, out=_step_tmp);   _step_tmp *= C2;            _step_n1 -= _step_tmp
    ...
    # ── New p2: covariant D²p2 = ∇²p2 − 2e(Ax·∂_xp1 + Ay·∂_yp1) − e²|A|²·p2 ──
    np.multiply(C2, _lap2, out=_step_n2)
    np.multiply(Ax, _step_dp1x, out=_step_tmp);  _step_tmp *= (_2E_W * C2);  _step_n2 -= _step_tmp
    np.multiply(Ay, _step_dp1y, out=_step_tmp);  _step_tmp *= (_2E_W * C2);  _step_n2 -= _step_tmp
    np.multiply(_step_Anx, p2, out=_step_tmp);   _step_tmp *= C2;            _step_n2 -= _step_tmp
    ...
    # ── Gauge field step ──────────────────────────────────────────────────────
    # Supercurrent at old time level (p1P, p2P = field just before scalar update,
    # Ax, Ay = current gauge field at same time):
    # Jx = e·(p1P·∂_xp2_old − p2P·∂_xp1_old − e·Ax·|φ_old|²)
    # Jy = e·(p1P·∂_yp2_old − p2P·∂_yp1_old − e·Ay·|φ_old|²)

    # |φ_old|² → _step_n1 (free)
    np.multiply(p1P, p1P, out=_step_n1)
    np.multiply(p2P, p2P, out=_step_tmp);  _step_n1 += _step_tmp

    # Jx
    np.multiply(p1P, _step_dp2x, out=_step_Jx)
    np.multiply(p2P, _step_dp1x, out=_step_tmp);  _step_Jx -= _step_tmp
    np.multiply(Ax,  _step_n1,   out=_step_tmp);  _step_tmp *= E_W;  _step_Jx -= _step_tmp
    _step_Jx *= E_W

    # Jy
    np.multiply(p1P, _step_dp2y, out=_step_Jy)
    np.multiply(p2P, _step_dp1y, out=_step_tmp);  _step_Jy -= _step_tmp
    np.multiply(Ay,  _step_n1,   out=_step_tmp);  _step_tmp *= E_W;  _step_Jy -= _step_tmp
    _step_Jy *= E_W

    # B = ∂_x Ay − ∂_y Ax (interior; _step_n2 is temp)
    _step_B[:] = 0.0;  _step_n2[:] = 0.0
    np.subtract(Ay[1:-1, 2:], Ay[1:-1, :-2], out=_step_B[1:-1, 1:-1]);  _step_B[1:-1, 1:-1] *= 0.5
    np.subtract(Ax[2:, 1:-1], Ax[:-2, 1:-1], out=_step_n2[1:-1, 1:-1]); _step_n2[1:-1, 1:-1] *= 0.5
    _step_B[1:-1, 1:-1] -= _step_n2[1:-1, 1:-1]

    # Gauge velocities
    np.subtract(Ax, AxP, out=_step_v1)
    np.subtract(Ay, AyP, out=_step_v2)

    # New Ax = (1+d)·Ax − d·AxP + DT²·(−∂_y B + Jx) − pml·vAx
    _step_n2[:] = 0.0
    np.subtract(_step_B[2:, 1:-1], _step_B[:-2, 1:-1], out=_step_n2[1:-1, 1:-1]); _step_n2[1:-1, 1:-1] *= 0.5
    np.negative(_step_n2, out=_step_Anx);  _step_Anx += _step_Jx
    _step_Anx *= _DT2
    np.multiply(1.0+d, Ax,  out=_step_tmp);  _step_Anx += _step_tmp
    np.multiply(d,     AxP, out=_step_tmp);  _step_Anx -= _step_tmp
    if not S['reflect']:
        np.multiply(pml, _step_v1, out=_step_tmp);  _step_Anx -= _step_tmp
    _step_Anx[[0,-1], :] = 0.0;  _step_Anx[:, [0,-1]] = 0.0

    # New Ay = (1+d)·Ay − d·AyP + DT²·(+∂_x B + Jy) − pml·vAy
    _step_n2[:] = 0.0
    np.subtract(_step_B[1:-1, 2:], _step_B[1:-1, :-2], out=_step_n2[1:-1, 1:-1]); _step_n2[1:-1, 1:-1] *= 0.5
    np.copyto(_step_Any, _step_n2);  _step_Any += _step_Jy
    _step_Any *= _DT2
    np.multiply(1.0+d, Ay,  out=_step_tmp);  _step_Any += _step_tmp
    np.multiply(d,     AyP, out=_step_tmp);  _step_Any -= _step_tmp
    if not S['reflect']:
        np.multiply(pml, _step_v2, out=_step_tmp);  _step_Any -= _step_tmp
    _step_Any[[0,-1], :] = 0.0;  _step_Any[:, [0,-1]] = 0.0

    AxP[:] = Ax;  AyP[:] = Ay
    Ax[:] = _step_Anx;  Ay[:] = _step_Any
...
def place_vortex(ci, cj, charge):
    ...
    p1P[:] = p1;  p2P[:] = p2
    # London gauge profile: Ax = −q/e·f·dr/r², Ay = +q/e·f·dc/r²
    # f(r) = 1 − exp(−r²/λ_L²) — Gaussian onset, finite at r=0, → 1 far out
    r2 = dr*dr + dc*dc
    f  = (1.0 - np.exp(-r2 / _LAM_L2)).astype(np.float32)
    Ax[:] += (-charge / E_W) * f * dr / (r2 + 1e-6)
    Ay[:] += ( charge / E_W) * f * dc / (r2 + 1e-6)
    AxP[:] = Ax;  AyP[:] = Ay
...
def save_undo():
    _undo_stack.append((p1.copy(), p2.copy(), p1P.copy(), p2P.copy(),
                        Ax.copy(), Ay.copy(), AxP.copy(), AyP.copy(),
...
def do_undo():
    ...
    if not _undo_stack:
        return
    p1c, p2c, p1Pc, p2Pc, Axc, Ayc, AxPc, AyPc, srcs, nv = _undo_stack.pop()
    p1[:] = p1c;  p2[:] = p2c;  p1P[:] = p1Pc;  p2P[:] = p2Pc
    Ax[:] = Axc;  Ay[:] = Ayc;  AxP[:] = AxPc;  AyP[:] = AyPc
...
def do_load(e=None):
    ...
    try:
        ...
        p1[:]  = d['p1'];   p2[:]  = d['p2']
        p1P[:] = d['p1P'];  p2P[:] = d['p2P']
        Ax[:]  = d['Ax']  if 'Ax'  in d else 0.0
        Ay[:]  = d['Ay']  if 'Ay'  in d else 0.0
        AxP[:] = d['AxP'] if 'AxP' in d else 0.0
        AyP[:] = d['AyP'] if 'AyP' in d else 0.0
        S['lam'] = float(d['lam'])
        S['nv']  = int(d['nv'])
        spin_sources.clear()
        if 'src_ci' in d:
...
def to_rgb():
    ...
    if mode == 'phase':
        ...
        # e = 0.5·grad² + λ·(mod²−1)² + 0.5·B² → reuse _s_amp
        _s_ph -= 1.0
        np.multiply(_s_ph, _s_ph, out=_s_tmp);  _s_tmp *= S['lam']
        _s_amp *= 0.5;  _s_amp += _s_tmp
        # add magnetic energy ½B² where B = ∂_x Ay − ∂_y Ax
        np.subtract(Ay[1:-1, 2:], Ay[1:-1, :-2], out=_s_r[1:-1, 1:-1]); _s_r[1:-1, 1:-1] *= 0.5
        np.subtract(Ax[2:, 1:-1], Ax[:-2, 1:-1], out=_s_g[1:-1, 1:-1]); _s_g[1:-1, 1:-1] *= 0.5
        _s_r[1:-1, 1:-1] -= _s_g[1:-1, 1:-1]
        np.multiply(_s_r, _s_r, out=_s_g);  _s_g *= 0.5;  _s_amp += _s_g
...
try:
    fig.canvas.manager.set_window_title('Abelian-Higgs Vortex Simulation  v4.18')
except Exception:
    pass
...
b_reflect = mkbtn([_sx, 0.175, _aw, _ah], 'REFLECT: OFF', 8)
b_quit    = mkbtn([_sx, 0.113, _aw, _ah], 'QUIT')
...
# Legend
ax_leg = fig.add_axes([_sx, 0.000, _aw, 0.107], facecolor='#080808')
for k, (txt, col) in enumerate([
    ...
    ax_leg.text(0.04, 0.93 - k*0.19, txt, color=col,
...
print("Abelian-Higgs Vortex Simulation v4.18 (PDG 2025 constants)")
