#!/usr/bin/env python3
"""
Abelian-Higgs Vortex Simulation  —  Claude Code edition  v4.1
==============================================================
Changes vs v4:
  - Performance: scipy.ndimage.convolve replaced by a numpy-roll
    5-point Laplacian.  Same stencil, ~2× faster at N=256, no scipy
    dependency.  Speed slider ceiling raised from 12 → 16 steps/frame.

    Old:  convolve(f, _K5, mode='wrap')        ← general conv overhead
    New:  roll(f,1,0)+roll(f,-1,0)
         +roll(f,1,1)+roll(f,-1,1) − 4f       ← pure numpy, SIMD-friendly
...
try:
    fig.canvas.manager.set_window_title('Abelian-Higgs Vortex Simulation  v4.1')
except Exception:
    pass
...
sl_spf   = mk_slider(0.491, 'speed',     1,  16,  3,        step=1)   # ceiling 12→16
...
print("Abelian-Higgs Vortex Simulation v4.1 (numpy-roll Laplacian)")
