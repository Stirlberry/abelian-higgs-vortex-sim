#!/usr/bin/env python3
"""
Abelian-Higgs Vortex Simulation  —  Claude Code edition  v4.17aa
================================================================
Changes vs v4.17a:
  - Save / load field state: SAVE button writes p1, p2, p1P, p2P + λ + spin
    sources to a .npz file via Qt file dialog; LOAD restores and resyncs slider
  - Right-click anywhere in the sim always places a single antivortex (−1),
    regardless of the current block selector — left-click unchanged
...
Inherited from v4.2:
  ...
  - numpy-roll 5-point Laplacian (no scipy), speed ceiling 16 steps/frame
...
def step():
    ...
    d   = S['damp']
    n1 = (1.0+d)*p1 - d*p1P + dt2*(C2*_lap1 - pot*p1) - ab*v1 - 2.0*A0*v2
    n2 = (1.0+d)*p2 - d*p2P + dt2*(C2*_lap2 - pot*p2) - ab*v2 + 2.0*A0*v1
...
def to_rgb():
    ...
    if mode == 'phase':
        ...
        # Gradient of p1 and p2 (central difference) — 8 np.roll calls create
        # short-lived 256KB temps, but _s_r/g/b/tmp capture results in-place.
        np.subtract(np.roll(p1,-1,1), np.roll(p1,1,1), out=_s_r);   _s_r *= 0.5  # dx1
        np.subtract(np.roll(p1,-1,0), np.roll(p1,1,0), out=_s_g);   _s_g *= 0.5  # dy1
        np.subtract(np.roll(p2,-1,1), np.roll(p2,1,1), out=_s_b);   _s_b *= 0.5  # dx2
        np.subtract(np.roll(p2,-1,0), np.roll(p2,1,0), out=_s_tmp); _s_tmp *= 0.5  # dy2
        ...
        # Gradients of n1, n2 (central difference) — _s_amp=dn1x, _s_tmp=dn1y,
        #                                             _s_ph=dn2x,  _s_b=dn2y
        np.subtract(np.roll(_s_r,-1,1), np.roll(_s_r,1,1), out=_s_amp); _s_amp *= 0.5
        np.subtract(np.roll(_s_r,-1,0), np.roll(_s_r,1,0), out=_s_tmp); _s_tmp *= 0.5
        np.subtract(np.roll(_s_g,-1,1), np.roll(_s_g,1,1), out=_s_ph);  _s_ph  *= 0.5
        np.subtract(np.roll(_s_g,-1,0), np.roll(_s_g,1,0), out=_s_b);   _s_b   *= 0.5
...
try:
    fig.canvas.manager.set_window_title('Abelian-Higgs Vortex Simulation  v4.17a')
except Exception:
    pass
...
print("Abelian-Higgs Vortex Simulation v4.17a (PDG 2025 constants)")
