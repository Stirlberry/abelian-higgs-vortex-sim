#!/usr/bin/env python3
"""
Abelian-Higgs Vortex Simulation  —  Claude Code edition  v4.15bb
================================================================
Changes vs v4.14 (Option 2 — clip path, for comparison with v4.15b):
  - Circular clip path applied to img_h: matplotlib clips the image to a smooth
    vector circle at render time. Hard in_circle mask retained in to_rgb().
...
# Soft edge — smoothstep fade over outermost 3 px feeds into the clip path
_t         = np.clip((N // 2 - 2 - R_GRID) / 3.0, 0.0, 1.0)
_soft_edge = (_t * _t * (3.0 - 2.0 * _t)).astype(np.float32)

# 5-point Laplacian via numpy roll — no scipy, same stencil as v4
def lap(f):
...
def step():
    ...
    d   = S['damp']
    n1 = (1.0+d)*p1 - d*p1P + dt2*(C2*l1 - pot*p1) - ab*v1 - 2.0*A0*v2
    n2 = (1.0+d)*p2 - d*p2P + dt2*(C2*l2 - pot*p2) - ab*v2 + 2.0*A0*v1
...
def to_rgb():
    ...
    if mode == 'phase':
        ...
        dx1 = (np.roll(p1, -1, 1) - np.roll(p1, 1, 1)) * 0.5
        dy1 = (np.roll(p1, -1, 0) - np.roll(p1, 1, 0)) * 0.5
        dx2 = (np.roll(p2, -1, 1) - np.roll(p2, 1, 1)) * 0.5
        dy2 = (np.roll(p2, -1, 0) - np.roll(p2, 1, 0)) * 0.5
        grad2 = dx1**2 + dy1**2 + dx2**2 + dy2**2
        mod2  = p1*p1 + p2*p2
        e     = np.clip(0.5 * grad2 + S['lam'] * (mod2 - 1.0)**2, 0, None)
        e_max = float(e[in_circle].max()) or 1.0
        t     = np.clip(e / e_max, 0, 1)
        # Hot colourmap: black → red → yellow → white
        r = np.clip(t * 3.0,       0, 1)
        ...
        mod  = np.sqrt(p1*p1 + p2*p2) + 1e-10
        n1   = p1 / mod;  n2 = p2 / mod
        dn1x = (np.roll(n1, -1, 1) - np.roll(n1, 1, 1)) * 0.5
        dn1y = (np.roll(n1, -1, 0) - np.roll(n1, 1, 0)) * 0.5
        dn2x = (np.roll(n2, -1, 1) - np.roll(n2, 1, 1)) * 0.5
        dn2y = (np.roll(n2, -1, 0) - np.roll(n2, 1, 0)) * 0.5
        q    = (dn1x * dn2y - dn1y * dn2x) / (2.0 * np.pi)
        qs   = max(float(np.abs(q[in_circle]).max()), 1e-6)
        qn   = np.clip(q / qs, -1, 1)
        r    = np.clip(-qn, 0, 1)
        g    = np.zeros_like(qn)
        b    = np.clip( qn, 0, 1)
...
try:
    fig.canvas.manager.set_window_title('Abelian-Higgs Vortex Simulation  v4.15b')
except Exception:
    pass
...
_clip_circle = Circle((CX, CY), N // 2 - 0.5, transform=ax.transData)
img_h.set_clip_path(_clip_circle)
...
def update(frame):
    ...
    return [img_h, info_txt, core_plus, core_minus, quiver_h]
...
print("Abelian-Higgs Vortex Simulation v4.15b (PDG 2025 constants)")
